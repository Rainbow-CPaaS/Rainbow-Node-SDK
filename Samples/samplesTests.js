...
const ts_generic_collections_linq_1 = require("ts-generic-collections-linq");
...
class ItemForQueue {
    constructor(itemFunction) {
        let that = this;
        that.defered = new Utils_1.Deferred();
...
class SamplesTests {
    constructor(logger) {
        ...
        that.listRequests = new ts_generic_collections_linq_1.List();
        ...
        try {
            ...
            that.lock(async () => {
                try {
                    ...
                    let deferedResult = await deferedItem.start();
    ...
    async treat() {
        ...
        while (that.listRequests.length > 0) {
            try {
                let item = that.listRequests.elementAt(0);
                if (!item)
                    return;
...
commandLineInteraction();
//# sourceMappingURL=samplesTests.js.map