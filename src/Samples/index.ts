...
// Later in the code
    ...
    class Tests {
     ...
     "internal" = 8,
        ...
        /**
         * Test for ImsService.sendForwardMessageToJid
         * Sends a message to vincent02 then forwards it to vincent03.
         * Usage from CLI: tests.testSendForwardMessage()
         */
        async testSendForwardMessage() {
            let contactEmailVincent02 = "vincent02@vbe.test.openrainbow.net";
            let contactEmailVincent03 = "vincent03@vbe.test.openrainbow.net";

            _logger.log("debug", "MAIN - testSendForwardMessage - starting");

            try {
                // 1. Send initial message to Vincent02
                let contact02 = await rainbowSDK.contacts.getContactByLoginEmail(contactEmailVincent02);
                if (!contact02) {
                    _logger.log("error", "MAIN - testSendForwardMessage - Vincent02 not found");
                    return;
                }

                let now = new Date().getTime();
                let msgSentTo02 = await rainbowSDK.im.sendMessageToJid("Message to be forwarded, timestamp: " + now, contact02.jid, "FR", null, "Le sujet de node : " + now, UrgencyType.STANDARD, DataStoreType.StoreTwinSide);
                _logger.log("debug", "MAIN - testSendForwardMessage - message sent to Vincent02", msgSentTo02);

                // 2. Forward this message to Vincent03
                let contact03 = await rainbowSDK.contacts.getContactByLoginEmail(contactEmailVincent03);
                if (!contact03) {
                    _logger.log("error", "MAIN - testSendForwardMessage - Vincent03 not found");
                    return;
                }

                let msgForwardedTo03 = await rainbowSDK.im.sendForwardMessageToJid(contact03.jid, msgSentTo02);
                _logger.log("debug", "MAIN - testSendForwardMessage - message forwarded to Vincent03", msgForwardedTo03);
            } catch (err) {
                _logger.log("error", "MAIN - testSendForwardMessage - error:", err);
            }
        }
          ...
          //  const card = { /* votre Adaptive Card minifiée */ };
            ...
            let msgSent : any = await rainbowSDK.im.sendMessageToConversation(conversation, "Test adptaptive card big size : " + contentMessageSize, "en", content, undefined).then((msgSent) => {
                ...
                try {
                    ...
                    if (looksLikeServer) {
                        _logger.log("warn", "MAIN - testsendChatMessageWithContentAdaptiveCardBigSize - Server-side XMPP error stanza detected (likely stanza too large). Details : ", errObj);
                    } else {
                        _logger.log("warn", "MAIN - testsendChatMessageWithContentAdaptiveCardBigSize - Local/client error or transport issue (no server error stanza markers found). Details : ", errObj);
                    }
                    //_logger.log("warn", "MAIN - testsendChatMessageWithContentAdaptiveCardBigSize - Error summary : ",  errObj);
                } catch (e) {
                    _logger.log("warn", "MAIN - testsendChatMessageWithContentAdaptiveCardBigSize - Error while classifying error : ", e);
                }
                _logger.log("warn", "MAIN - testsendChatMessageWithContentAdaptiveCardBigSize - Error summary : ", err);
            });
        }

        testescapeXMLText(text : string = undefined){
            const { escapeXML, escapeXMLText } = require("ltx/lib/escape.js");

            if (!text) {
                //text = `"{"$schema":"http://adaptivecards.io/schemas/adaptive-card.json","type":"AdaptiveCard","version":"1.6","body":[{"type":"TextBlock","text":"📋 Liste des employés"`;
                text = this.formatCard4();
            }

            let escapedText = escapeXML(text);
            _logger.log("debug", "MAIN - testescapeXMLText - result escapedText : ", escapedText);
...
<ping xmlns='urn:xmpp:ping'/>
    ...
    <iq type='result'
      ...
      <query xmlns='http://jabber.org/protocol/disco#info'>
        ...
        //region Start / Stop
        async testStartWithToken(token : string = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..aP4LC9HX-QO1s9gf68-R08goe4472YQYEOErRc7_piaVRRPYchD6Fo3u3CXJNmwep5MJjnypuJKlttQ4mtMRHG5np3b_1peARj0qqMpePag4JiQZWV9ne9DwcwNRhxD8uTmYEDOezGH8hhpIvkqUfuHpR4ZW7Anff5SeVOHPWzwcJ5EUJQKQKKR3sEfEC_2PHd7fywEw0BDOxCIXFQjC1jG3_JbIgnIGOqTwOFdH9-ZaurDjj9mU2JL4l9GKPn_afi1YiBjoAm3Er7hM-x6XwHHdJBvl49SY-4p7uzhqFIFNnrZ-73Cihbo8RTyb0hnCdOB36p6HfiVytL6UwZHQCw") {
            await rainbowSDK.stop();
