    ...
    getNomadicStatus(requestHeader) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            that.http.get("/api/rainbow/telephony/v1.0/nomadic", requestHeader).then((json) => {
                ...
                that.logger.log("info", LOG_ID + "(getNomadicStatus) REST conversation consulted", json.data);
                that.logger.log("debug", LOG_ID + "(getNomadicStatus) _exiting_");
                resolve(json.data);
            }).catch((err) => {
                that.logger.log("error", LOG_ID, "(getNomadicStatus) error", err);
                that.logger.log("debug", LOG_ID + "(getNomadicStatus) _exiting_");
                reject(err);
            });
        });
    ...
    nomadicLogin(requestHeader, data) {
        ...
        return new Promise((resolve, reject) => {
            ...
            that.http.put("/api/rainbow/telephony/v1.0/nomadic/login", requestHeader, data).then((json) => {
                ...
                that.logger.log("info", LOG_ID + "(nomadicLogin) REST nomadic login succeed", json.data);
                that.logger.log("debug", LOG_ID + "(nomadicLogin) _exiting_");
                resolve(json.data);
            }).catch((err) => {
                that.logger.log("error", LOG_ID, "(nomadicLogin) error", err);
                that.logger.log("debug", LOG_ID + "(nomadicLogin) _exiting_");
                reject(err);
            });
        });
