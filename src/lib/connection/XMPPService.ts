...
// */
        ...
        await that.xmppClient.init(that._logger, that.eventEmitter, that.timeBetweenXmppRequests, that.storeMessages, that.rateLimitPerHour, that.messagesDataStore, that.copyMessage, that.enablesendurgentpushmessages);
        ...
        that.xmppClient.on("input", function fn_input (stanzaStr) {
            ...
            if ( that._logger.enableEncryptedLogs == true ) {
                ...
                that._logger.log(that.INFO, LOG_ID + "(handleXMPPConnection) ", " raw in - encoded : (" + encodedXml + ")");
            }
        ...
        that.xmppClient.on("output", function fn_output (stanzaStr) {
            ...
            if ( that._logger.enableEncryptedLogs == true ) {
                let encodedXml = that._logger.encrypt(xmlStr);
                that._logger.log(that.INFO, LOG_ID + "(handleXMPPConnection) ", " raw out - encoded : (" + encodedXml + ")");
            }
