...
-   Fix `Message.ts` / `conversationEventHandler.ts`: the `action` and `type` attributes of an incoming `<answeredMsg>` XMPP element (sent by a Rainbow client when a recipient acknowledges or ignores an "emergency"/urgent message) were parsed for the message id and stamp but silently dropped for `action`/`type`, leaving applications with no reliable, locale-independent way to distinguish an acknowledgment from an ignore. Added `Message.answeredMsgAction` (`"ack"` or `"ignore"`) and `Message.answeredMsgType` (e.g. `"urgent"`), populated at all 3 places where an incoming `<answeredMsg>` is parsed. Verified end-to-end against real Rainbow iOS and Web clients (CRRAINB-53668).
-   Add a `warn`-level log in `XmppClient.ts` / `send()` when sending an "emergency" (`urgency=high`) message whose `messagesDataStore` resolves to a non-`Store` value (e.g. the SDK-wide default `DataStoreType.NoStore`): such a message is not durably retained server-side, which can prevent recipients from retrieving it later (mobile client catching up via push notification while locked, or simply re-opening the conversation), making the message appear unavailable and any subsequent acknowledgment unusable by the sending application. Related to CRRAINB-53668 (root cause of the originally reported symptom; the actual fix is for the consuming application to pass `DataStoreType.Store`/`StoreTwinSide` explicitly for messages requiring reliable delivery).
...
-   Fix `LevelLogs.ts` / `BackendStatusService`: `backendstatus` log accessor was missing from the `LawLayer` interface, constructor, `configureRESTLevelLogs()`, `toJSON()`, and getter/setter — log level configuration for the new service silently failed. Also remove stale `Utils.ts` references.
...
#### Changed
-   Refactor `RESTService.ts`: extract all HTTP wrapper methods into 41 dedicated `GenericRESTService` sub-classes under `src/lib/connection/RestServices/` (`RESTAdLdap`, `RESTAlerts`, `RESTApiSettings`, `RESTApplications`, `RESTAuth`, `RESTBots`, `RESTBubbleOpenInvites`, `RESTBubbles`, `RESTBubblesDialIn`, `RESTBubblesTags`, `RESTCalendar`, `RESTChannels`, `RESTClientsVersions`, `RESTCloudPbx`, `RESTConference`, `RESTConferenceV2`, `RESTConnectors`, `RESTContacts`, `RESTConversations`, `RESTCountry`, `RESTCustomerCare`, `RESTCustomisationTemplate`, `RESTDirectory`, `RESTFileStorage`, `RESTGroups`, `RESTInvitations`, `RESTPolls`, `RESTPresence`, `RESTProfiles`, `RESTPublicUrl`, `RESTRainbowVoice`, `RESTRoom`, `RESTS2S`, `RESTSettings`, `RESTSites`, `RESTSubscriptions`, `RESTSystems`, `RESTTasks`, `RESTTelephony`, `RESTWebinar`). Each sub-class extends `GenericRESTService` with `@logEntryExit(LOG_ID)`, is wired into `RESTService` startup/stop chain and all 5 credential setters. `RESTService` retains one-liner proxy methods. Zero breaking API changes.
-   Register all 41 REST sub-service log accessors in `LevelLogs.ts` (`LawLayer` interface, constructor, `configureRESTLevelLogs`, `toJSON`, getter/setters) and in `Samples/index.ts` `logLevelAreas` JSON config, enabling fine-grained per-sub-service log level control.
-   Add `// API https://...` and `// VERB /api/...` endpoint documentation comments to every public method in all 41 REST sub-service files; propagate the same comments to the corresponding proxy one-liners in `RESTService.ts`.
...
-   Tighten TypeScript return types on service methods with ErrorManager guard blocks: `ChannelsService` / `createItem` (`Promise<{}>` → `Promise<Err>`); `TasksService` / `addTask`, `getTaskById` (`Promise<any>` → `Promise<Task | Err>`); `TelephonyService` / `makeSimpleCall` private (`Promise<any>` → `Promise<Call | Err>`). Adds `Err` to `ErrorManager` imports in `BubblesService`, `ChannelsService`, `TasksService`, `TelephonyService`, `WebinarsService`. Fixes consumer type narrowing in `Samples/index.ts` / `testTask` (`addTask` result narrowed with `"id" in resultTask` guard before accessing `.id`).
