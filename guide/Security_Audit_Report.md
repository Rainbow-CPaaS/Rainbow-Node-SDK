...
| `jsdoc-x` | 6 (4 high, 1 moderate, 1 low) | Moved to development-only dependencies. It was incorrectly declared as a runtime dependency while only being used by an internal documentation-generation task, never by the SDK library code. Same effect as above: no longer installed for anyone consuming the SDK. |

**Net effect for a normal SDK consumer: ~49 of the 67 findings are eliminated entirely.**
