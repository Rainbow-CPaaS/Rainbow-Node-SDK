...
class Logger {
    ...
    constructor(config) {
        ...
        if (("logs" in config) && ("http" in config.logs))  {
            logHttp = config.logs.http;
        }
        ...
        this._logger.info = function () {
            that._winston.log.apply(that._winston, ["info", that._logger.customLabel + argumentsToString(arguments)]);
        };

        this._logger.error = function () {
            that._winston.log.apply(that._winston, ["error", that._logger.customLabel + argumentsToString(arguments)]);
        };

        this._logger.warn = function () {
            that._winston.log.apply(that._winston, ["warn", that._logger.customLabel + argumentsToString(arguments)]);
        };
