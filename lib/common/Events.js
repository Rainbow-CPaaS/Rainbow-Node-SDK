...
class Events {
    ...
    constructor( _logger, _filterCallback) {
        ...
        this._evReceiver.on("rainbow_reconnectionok", function() {
            var success = Error.OK;

            that._evPublisher.emit("rainbow_onreconnectionok", success);
        });

        this._evReceiver.on("rainbow_xmppreconnectingattempt", function() {
            var success = Error.OK;

            that._evPublisher.emit("rainbow_onreconnectingattempt", success);
        });

        this._evReceiver.on("rainbow_xmppdisconnect", function() {
            var success = Error.OK;

            that._evPublisher.emit("rainbow_ondisconnection", success);
        });
