...
class Events {
    ...
    constructor( _logger, _filterCallback) {
        ...
        this._evReceiver.on("rainbow_channelcreated", function (data) {
            /**
             * @event Events#rainbow_channelcreated
             * @public
             * @param { data }
             * @description
             *      Fired when a channel created event is received
             */
            that._logger.log("debug", LOG_ID + "publish event " + that._logger.colors.events("rainbow_channelcreated"));
            that._evPublisher.emit("rainbow_channelcreated", data);
        });

        this._evReceiver.on("rainbow_channeldeleted", function (data) {
            /**
             * @event Events#rainbow_channeldeleted
             * @public
             * @param { data }
             * @description
             *      Fired when a channel deleted event is received
             */
            that._logger.log("debug", LOG_ID + "publish event " + that._logger.colors.events("rainbow_channeldeleted"));
            that._evPublisher.emit("rainbow_channeldeleted", data);
        });
