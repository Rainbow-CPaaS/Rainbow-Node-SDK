...
class Conversation {
    ...
    constructor(conversationId) {
        ...
        /**
         * @public
         * @property {ConversationType} type The type of the conversation. Can be ONE_TO_ONE (0), ROOM (1) or BOT (2)
         * @readonly
         */
        this.type = null;
        ...
        /**
         * @public
         * @property {Room} room (ROOM conversation only) The conversation room
         * @link Room
         * @readonly
         */
        this.room = null;
    ...
    static createOneToOneConversation(participant) {
        console.info("[Conversation] Create one to one conversation (" + participant.id + ")");
    ...
    static createRoomConversation(room) {
        console.info("[Conversation] Create room conversation (" + room.jid + ")");
    ...
    /**
         ...
         * @instance
         * @description
         *    Return true if the conversation is engaged in a call (webRTC of Telephony)
    ...
    isInCall() {
        if (this.pstnConferenceSession) {
            return this
                ...
                .isParticipantConnectedByJid(contactService.userContact.jid);
        }
     ...
     */
    isNotInCall() {
        return (!this.videoCall || !this.videoCall.status || this.videoCall.status.value === "Unknown") && 
        (!this.audioCall || !this.audioCall.status || this.audioCall.status.value === "Unknown") && 
        (contactService.userContact.status !== "busy" || contactService.userContact.message === "");
    }

    isActive() {
        return (this.videoCall && this.videoCall.status && this.videoCall.status.value === "active") ||
            (this.audioCall && this.audioCall.status && this.audioCall.status.value === "active");
    }

    isHeld() {
        return (this.videoCall && this.videoCall.status && this.videoCall.status.value === "held") ||
            (this.audioCall && this.audioCall.status && this.audioCall.status.value === "held");
    }
