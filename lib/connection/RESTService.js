...
class RESTService {
    ...
    getContacts(listOfJid) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (listOfJid.length === 0) {
                that.logger.log("debug", LOG_ID + "(getContacts) successfull");
                that.logger.log("info", LOG_ID + "(getContacts) No contacts");
                ...
                var jidList = [];
                listOfJid.forEach((jidItem) => {
                    jidList.push(String(String(jidItem.jid)));
                });

                that.http.post("/api/rainbow/enduser/v1.0/users/jids?format=full&limit=" + listOfJidNumber, getRequestHeader(), { "jid_im": jidList } ).then(function(list) {
                    that.logger.log("debug", LOG_ID + "(getContacts) successfull");
    ...
    getContactInformationByJID(jid) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.logger.log("debug", LOG_ID + "(getContact) _entering_");

            if (!jid) {
                that.logger.log("debug", LOG_ID + "(getContact) successfull");
                that.logger.log("info", LOG_ID + "(getContact) No jid provided");
                that.logger.log("debug", LOG_ID + "(getContact) _exiting_");
                resolve(null);
            }
            else {
                that.http.get("/api/rainbow/enduser/v1.0/users/jids/" + encodeURIComponent(jid), getRequestHeader() ).then(function(json) {
                    that.logger.log("debug", LOG_ID + "(getContact) successfull");
                    that.logger.log("info", LOG_ID + "(getContact) REST contact received ", json.data);
                    that.logger.log("debug", LOG_ID + "(getContact) _exiting_");
                    resolve(json.data);
                }).catch(function(err) {
                    that.logger.log("error", LOG_ID, "(getContacts) error", err);
