...
const RESTTelepony = require('./RestServices/RESTTelephony').RESTService;
...
class RESTService {
    ...
    constructor(_credentials, _application, _isOfficialRainbow, evtEmitter, _logger) {
        ...
        this.restTelephony = RESTTelepony(evtEmitter, _logger);
        ...
        this.getPostHeaderWithRange = (accept, initialSize, minRange, maxRange) => {
            var header = this.getRequestHeader(accept);
			header.Range = "bytes " + minRange + "-" + maxRange + "/" + initialSize;
			return header;
        };
    ...
    uploadAFile(fileId, buffer) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.http.put( "/api/rainbow/fileserver/v1.0/files/" + fileId, that.getRequestHeader("application/octet-stream"), buffer).then(function(response) {
    ...
    sendPartialDataToServer(fileId, file, initialSize, minRange, maxRange, index) {
        let that = this;
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.http.put( "/api/rainbow/fileserver/v1.0/files/" + fileId + "/parts/" + index, that.getPostHeaderWithRange("application/octet-stream", initialSize, minRange, maxRange ), file).then(function(response) {
    ...
    sendPartialFileCompletion(fileId) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.http.put( "/api/rainbow/fileserver/v1.0/files/" + fileId + "/parts/end", that.getRequestHeader("application/octet-stream")).then(function(response) {
                that.logger.log("info", LOG_ID + "(sendPartialFileCompletion) successfull");
                that.logger.log("debug", LOG_ID + "(sendPartialFileCompletion) _exiting_");
                resolve( response);
            }).catch(function(err) {
