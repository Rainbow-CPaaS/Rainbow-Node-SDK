...
class Connection {
    ...
    startTokenSurvey() {
        ...
        if (tokenExpirationDuration < 0) {
            this.logger.log("warn", LOG_ID + "_startTokenSurvey - Auth token has already expired, re-new it immediately");
            that._renewAuthToken();
        }
        else if (tokenExpirationDuration < 300000) {
            this.logger.log("warn", LOG_ID + "_startTokenSurvey - Auth token will expire in less 5 minutes, re-new it immediately");
            that._renewAuthToken();
        }
        else {
            var usedExpirationDuration = tokenExpirationDuration - 3600000; // Refresh 1 hour before the token expiration
            this.logger.log("info", LOG_ID + "_startTokenSurvey - Start token survey (expirationDate: " + expirationDate + " tokenExpirationDuration: " + tokenExpirationDuration + "ms usedExpirationDuration: " + usedExpirationDuration + "ms)");
    ...
    _renewAuthToken() {
        ...
        this.logger.log("debug", LOG_ID + "_renewAuthToken - begin");
        ...
        that.http.get('/api/rainbow/authentication/v1.0/renew', getRequestHeader()).then(function(JSON) {
            ...
            that.logger.log("debug", LOG_ID + "_renewAuthToken - end");
    ...
    getContacts(listOfJid) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.logger.log("debug", LOG_ID + "getContacts - begin");
            ...
            that.http.post('/api/rainbow/enduser/v1.0/search?format=full&limit=' + listOfJidNumber, getRequestHeader(),{ "jid_im": jidList  } ).then(function(list) {
                 ...
                 that.logger.log("debug", LOG_ID + "getContacts - end");
                 resolve(list);
            }).catch(function(err) {
