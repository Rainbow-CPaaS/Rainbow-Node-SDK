    ...
    constructor(xmppService, conversationService) {
            ...
            children.forEach((node) => {
                switch (node.getName()) {
                    ...
                    case "paused":
                        ...
                        that.logger.log("info", LOG_ID + "(handleXMPPConnection) message - someone is " + node.getName());
        ...
        this.onFileManagementMessageReceived = (node) => {
            if (node.attrs.xmlns === "jabber:iq:configuration") {
                switch (node.attrs.action) {
                    case "create": {
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) file created");
                        ...
                        break;
                    case "update": {
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) file updated");
                    ...
                    case "delete": {
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) file deleted");
            ...
            if (node.attrs.xmlns === "jabber:iq:configuration") {
                switch (node.attrs.action) {
                    case "create": {
                        ...
                        that.eventEmitter.emit("rainbow_thumbnailcreated", {'url': url, 'mime': mime, 'filename': filename, 'size': size, 'md5sum': md5sum, 'fileid': fileid});
                    }
                        break;
                    default:
                        break;
        ...
        this.onChannelManagementMessageReceived = (node) => {
            if (node.attrs.xmlns === "jabber:iq:configuration") {
                switch (node.attrs.action) {
                    case "add": {
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) channel created");
                        let channelid = node.attrs.channelid;
                        that.eventEmitter.emit("rainbow_channelcreated", {'id': channelid});
                    }
                        break;
                    case "delete": {
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) channel deleted");
                        let channelid = node.attrs.channelid;
                        that.eventEmitter.emit("rainbow_channeldeleted", {'id': channelid});
                    }
                        break;
                    case "update": {
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) channel updated");
                        let channelid = node.attrs.channelid;
                    ...
                    default: {
                        let channelid = node.attrs.channelid;
                        that.logger.log("info", LOG_ID + "(handleXMPPConnection) channel management event unknown : " + node.attrs.action + " for channel " + channelid);
                    }
                        break;
                }
            }
        ...
        {
            that.logger.log("error", LOG_ID + "(handleXMPPConnection) findChildren element : ", element, " name : ", element.getName());
            let json = {};
            //let result = null;
            ...
            if ( children.length > 0 ) {
                ...
                children.forEach((elemt) => {
                    if (typeof elemt.children === Array) {
                        that.logger.log("error", LOG_ID + "(handleXMPPConnection)  children.forEach Array : ", element, ", elemt : ", elemt);
                        childrenJson[elemt.getName()] = elemt.children[0];
                    }
                    that.logger.log("error", LOG_ID + "(handleXMPPConnection)  children.forEach element : ", element, ", elemt : ", elemt);
                    childrenJson[elemt.getName()] = this.findChildren(elemt);
                });
                return json;
            } else {
                that.logger.log("error", LOG_ID + "(handleXMPPConnection)  No children element : ", element);
        ...
        this.onHeadlineMessageReceived = (msg, stanza) => {
            ...
            if (!eventNode) {
                that.logger.log("error", LOG_ID + "(handleXMPPConnection) ERROR in onHeadlineMessageReceived eventNode is empty");
            ...
            if (!items) {
                that.logger.log("error", LOG_ID + "(handleXMPPConnection) ERROR in onHeadlineMessageReceived items is empty");
            ...
            if (!item) {
                that.logger.log("error", LOG_ID + "(handleXMPPConnection) ERROR in onHeadlineMessageReceived item is empty");
            ...
            if (!entry) {
                that.logger.log("debug", LOG_ID + "(handleXMPPConnection) onHeadlineMessageReceived entry is empty");
            ...
            switch (item.name) {
                case "retract": {
                    ...
                    if (messageId === null) {
                        that.logger.log("error", LOG_ID + "(handleXMPPConnection) channel retract received, but id is empty. So ignored.");
                    } else {
                        let message = {} ;
                        message.messageId = item.attrs.id ;
                        that.logger.log("debug", LOG_ID + "(handleXMPPConnection) channel retract received, for messageId " + message.messageId);
                    ...
                    if (entry) {
                        ...
                        if (Array.isArray(images)) {
                            images.forEach((image) => {
                                ...
                                if (id === null) {
                                    that.logger.log("error", LOG_ID + "(handleXMPPConnection) channel image entry received, but image id empty. So ignored.");
                                } else {
                        ...
                        that.logger.log("error", LOG_ID + "(handleXMPPConnection) channel entry received, but empty. It can not be parsed, so ignored.", stanza);
                    }
                }
                break;
                default: {
                    that.logger.log("debug", LOG_ID + "(handleXMPPConnection) channel unknown event " + item.name + " received");
                }
                    break;
