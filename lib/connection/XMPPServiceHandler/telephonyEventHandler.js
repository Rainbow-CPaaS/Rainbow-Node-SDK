...
/*
        ...
        this.onVoiceMessageEvent = function (messagingElem) {
            that.logger.log("debug", LOG_ID + "(onVoiceMessageEvent) _entering_", messagingElem);
                        ...
                        if (!profileService.isFeatureEnabled(profileService.getFeaturesEnum().TELEPHONY_VOICE_MAIL)) {
            ...
            that.logger.log("debug", LOG_ID + "(onUpDateCallEvent) _entering_", updatecallElem);
                        ...
                        return this.getCall(updatecallElem).then(function (call) {
                            ...
                            if (!config.permitSearchFromPhoneBook) { // <--- allow to permit search even if not the good profile
                                ...
                                if (!profileService.isFeatureEnabled(profileService.getFeaturesEnum().TELEPHONY_PHONE_BOOK)) {
 ...
 <nomadicStatus destination="23031" featureActivated="true" makeCallInitiatorIsMain="false" modeActivated="true"/>
        ...
        this.onNomadicStatusEvent = function (eventElem) {
            that.logger.log("debug", LOG_ID + "(onNomadicStatusEvent) _entering_", eventElem);
            ...
            this.telephonyService.updateNomadicData(nomadicstate);
