    ...
    constructor(_xmpp, _im, _application, _eventEmitter, _logger, _proxy) {
        ...
        this.handleXMPPConnection = (headers) => {
            ...
            this.xmppClient.on(STANZA_EVENT, (stanza) => {
                that.logger.log("info", LOG_ID + "(handleXMPPConnection) event - STANZA_EVENT : " + STANZA_EVENT + " |", stanza.toString());

                let eventId = that.hash + "." + stanza.getNS() + "." + stanza.getName() + (stanza.attrs.type ? "." + stanza.attrs.type : "");
                that.logger.log("debug", LOG_ID + "(handleXMPPConnection) event - eventId ", eventId);
        ...
        if (that.useXMPP) {
            ...
            return new Promise((resolve, reject) => {
                that
                    ...
                    resolve({from: this.jid_im, to: jid, type: "groupchat", id: id, date: new Date(), content: message});
                }).catch((err) => {
                    reject(err);
                });
            });
...
/*
            let url = config.restServerUrl + "/api/rainbow/fileserver/v1.0/files/" + fileDescriptor.id;
            xmppMessage.c("x", {
                "xmlns": "jabber:x:oob"
            })
                .c("url", {}, url)
...
// */
    ...
    sendChatExistingFSMessageToBubble (){
    ...
    voiceMessageQuery(jid) {
        ...
        return new Promise((resolve,reject) => {
            that.logger.log("debug", LOG_ID + "(handleXMPPConnection) voiceMessageQuery - 'stanza'", stanza.toString());
