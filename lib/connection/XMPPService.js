                            ...
                            children.forEach(function(node) {
                                switch (node.getName()) {
                                    ...
                                    case "body":
                                        ...
                                        break;
                                    case "request":
                                        // Only Acknowledge chat message (not for groupchat message)
                                        if(stanza.attrs.type === "chat") {
                                            ...
                                            that.logger.log("info", LOG_ID + "(handleXMPPConnection) answered - 'receipt received'", stanzaReceived.root().toString());
                                            that.xmppClient.send(stanzaReceived);
                                            //Acknowledge 'read'
                                            if(that.shouldSendReadReceipt) {
                                                ...
                                                that.logger.log("info", LOG_ID + "(handleXMPPConnection) answered - 'receipt read'", stanzaRead.root().toString());
                            ...
                            var fromJid = stanza.attrs.from;
                                ...
                                if(hasATextMessage && ((stanza.attrs.type === "groupchat" && fromBubbleUserJid !== that.fullJid) || (stanza.attrs.type === "chat" && fromJid !== that.fullJid))) {
                                    ...
                                    that.eventEmitter.emit('rainbow_onmessagereceived', {
                                        'fromJid': fromJid,
                                        'resource': resource,
                                        'fromBubbleJid': fromBubbleJid,
                                        'fromBubbleUserJid': fromBubbleUserJid,
