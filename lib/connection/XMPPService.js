            ...
            this.xmppClient.on('stanza', function(stanza) {
                ...
                switch(stanza.getName()) {
                    ...
                    case "presence":
                        ...
                        if (from === that.fullJid || getBareJIDFromFullJID(from) === getBareJIDFromFullJID(that.fullJid)) {
                            ...
                            that.eventEmitter.emit("rainbow_onpresencechanged", {
                                ...
                                show: 'online',
                                fromTelJid: isFromTelJid(from),
                                type: isFromTelJid(from) ? "phone" : isFromMobile(from) ? "mobile" : isFromNode(from) ? "node" : "desktopOrWeb"
                            });
                        }
                        else {
                            var priority = 5;
