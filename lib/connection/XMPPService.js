    ...
    constructor(_xmpp, _im, _application, _eventEmitter, _logger, _proxy) {
        ...
        this.handleXMPPConnection = (headers) => {
            ...
            this.xmppClient.on("input", (packet) => {
                that.logger.log("internal", LOG_ID + "(handleXMPPConnection) raw in - << " + packet );
                that.startOrResetIdleTimer(true);
            });

            this.xmppClient.on("output", (packet) => {
                that.logger.log("internal", LOG_ID + "(handleXMPPConnection) raw out - >> " + packet );
            ...
            this.xmppClient.on(ERROR_EVENT, function(err) {
                ...
                that.stopIdleTimer();
                if (that.reconnect && err.condition != "conflict") {
                    if( !that.isReconnecting ) {
    ...
    getAgentStatus () {
        ...
        return new Promise((resolve, reject) => {
            ...
            let stanza = xml("iq", {
                ...
                xmlns: "jabber:client",
                "id": XMPPUtils.getUniqueMessageId()
            }, xml("pbxagentstatus", {"xmlns": "urn:xmpp:pbxagent:monitoring:1"}));
...
// </iq>
            if (!secondary) {
                ...
                stanza = xml("iq", {
                    ...
                    xmlns: "jabber:client",
                    "id": XMPPUtils.getUniqueMessageId()
                }, xml("callservice", {"xmlns":  "urn:xmpp:pbxagent:callservice:1"}, xml("connections")));
                ...
                stanza = xml("iq", {
                    ...
                    //xmlns: "jabber:client",
                    "id": XMPPUtils.getUniqueMessageId()
                }, xml("callservice", {"xmlns":  "urn:xmpp:pbxagent:callservice:1"}, xml("connections", {deviceType: "SECONDARY"})));
            }
