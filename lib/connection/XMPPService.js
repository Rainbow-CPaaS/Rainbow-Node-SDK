...
class XMPPService {
    ...
    constructor(_xmpp, _eventEmitter, _logger) {
        ...
        handleXMPPConnection = () => {
            ...
            this.xmppClient.on('online', function(msg) {
                ...
                that.logger.log("debug", LOG_ID + "Connected as " + msg.jid);
                that.eventEmitter.emit("rainbow_xmppconnected");
            });

            this.xmppClient.on('stanza', function(stanza) {
                that.logger.log("debug", LOG_ID + "Received - 'stanza'", stanza.toString());
                        ...
                        children.forEach(function(node) {
                            switch(node.getName()) {
                                case "ping":
                                    ...
                                    that.logger.log("debug", LOG_ID + "Answered - 'stanza'", stanzaResponse.toString());
                                ...
                                case "query":
                                    if(stanza.attrs.type === "result") {
                                        if(node.attrs.xmlns === "jabber:iq:roster") {
                                            ...
                                            that.logger.log("debug", LOG_ID + "XMPP Rosters received", contacts.length);
                                            that.eventEmitter.emit('rainbow_onrosters', contacts);
                                        }
                                    }
                                    break;
                                case "default":
                                    that.logger.log("debug", LOG_ID + "Not managed - 'stanza'", node.getName());
                                    break;
                            }
                        });
                        break;
                            ...
                            children.forEach(function(node) {
                                switch (node.getName()) {
                                    ...
                                    case "body":
                                        ...
                                        that.logger.log("debug", LOG_ID + "message - content", content);
                                        break;
                                    case "request":
                                        ...
                                        that.logger.log("debug", LOG_ID + "Answered - 'stanza'", stanzaReceived.root().toString());
                                        ...
                                        that.logger.log("debug", LOG_ID + "Answered - 'stanza'", stanzaRead.root().toString());
            ...
            this.xmppClient.on('offline', function () {
                that.logger.log("warning", LOG_ID + "start - websocket on 'offline'");
            });

            this.xmppClient.on('connect', function (msg) {
                that.logger.log("debug", LOG_ID + "start - websocket on 'connect'", msg);
            });
            ...
            this.xmppClient.on('disconnect', function (e) {
                that.logger.log("warning", LOG_ID + "start - websocket on 'disconnect'", e);
            });

            this.xmppClient.on('register', function (e) {
                that.logger.log("debug", LOG_ID + "start - websocket on 'register'", e);
            });

            this.xmppClient.on('authenticate', function (e) {
                that.logger.log("debug", LOG_ID + "start - websocket on 'authenticate'", e);
            });

            this.xmppClient.on('connection', function (e) {
                that.logger.log("debug", LOG_ID + "start - websocket on 'connection'", e);
            });
        }
    }
    ...
    signin(account) {
        var that = this;
        this.logger.log("info", LOG_ID + "signin - begin");
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.logger.log("debug", LOG_ID + "signin - account used", that.jid_im);
            
            handleXMPPConnection();
            that.logger.log("info", LOG_ID + "signin - end");
            resolve();
        });
    }
    ...
    sendInitialPresence() {
        ...
        this.logger.log("debug", LOG_ID + "send - 'stanza'", stanza.toString());
    ...
    sendChatMessage(message, jid) {
        ...
        this.logger.log("debug", LOG_ID + "send - 'message'", stanza.toString());
        this.xmppClient.send(stanza);
        return {
            to: jid,
            message: 'chat',
    ...
    getRosters() {
        ...
        this.logger.log("debug", LOG_ID + "send - 'iq/rosters'", stanza.toString());
        this.xmppClient.send(stanza);
    }
};
