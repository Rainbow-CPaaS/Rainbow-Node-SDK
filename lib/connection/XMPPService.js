    ...
    constructor(_xmpp, _eventEmitter, _logger, _proxy) {
        ...
        handleXMPPConnection = () => {
            ...
            this.xmppClient.on('online', function(msg) {
                that.logger.log("info", LOG_ID + "Event - online");
                that.logger.log("debug", LOG_ID + "Received", msg);
            ...
            this.xmppClient.on('stanza', function(stanza) {
                that.logger.log("info", LOG_ID + "Event - stanza");
                that.logger.log("info", LOG_ID + "Received", stanza.toString());
            ...
            this.xmppClient.on('error', function (err) {
                ...
                that.logger.log("error", LOG_ID + "Received", err);
                that.xmppClient.end();
                that.eventEmitter.emit('rainbow_onxmpperror', err);
            });

            this.xmppClient.on('offline', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - offline");
                that.logger.log("warn", LOG_ID + "Received", msg);
            });

            this.xmppClient.on('connect', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - connect");
                that.logger.log("info", LOG_ID + "Received", msg);
            });

            this.xmppClient.on('reconnect', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - reconnect");
                that.logger.log("info", LOG_ID + "Received", msg);
            })

            this.xmppClient.on('disconnect', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - disconnect");
                that.logger.log("warn", LOG_ID + "Received", msg);
            });

            this.xmppClient.on('register', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - register");
                that.logger.log("info", LOG_ID + "Received", msg);
            });

            this.xmppClient.on('authenticate', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - authenticate");
                that.logger.log("info", LOG_ID + "Received", msg);
            });

            this.xmppClient.on('connection', function (msg) {
                that.logger.log("debug", LOG_ID + "Event - connection");
                that.logger.log("info", LOG_ID + "Received", msg);
            });
        }
    }
