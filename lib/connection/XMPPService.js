...
class XMPPService {
    ...
    constructor(_xmpp, _im, _eventEmitter, _logger, _proxy) {
        ...
        handleXMPPConnection = () => {
            ...
            this.reconnect = this
                .xmppClient
                .plugin(require("@xmpp/plugins/reconnect"));

            this.reconnect.delay = RECONNECT_INITIAL_DELAY;

            var fibonacciStrategy = new backoff.FibonacciStrategy({randomisationFactor: 0.4, initialDelay: RECONNECT_INITIAL_DELAY, maxDelay: RECONNECT_MAX_DELAY});

            this
                .xmppClient
                .plugin(require("./sasl-digest-md5"));

            this
                .xmppClient
                .handle(AUTHENTICATE_EVENT, authenticate => {
                    return authenticate(this.fullJid, this.jid_password);
                });

            this
                .xmppClient
                .handle(BIND_EVENT, (bind) => {
                    return bind(this.fullJid);
                });

            this
                .xmppClient
                .on(ONLINE_EVENT, function (msg) {
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) event - " + ONLINE_EVENT);
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) received", msg);
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) connected as " + msg);
                    that
                        .eventEmitter
                        .emit("rainbow_xmppconnected");
                });
            ...
            this
                ...
                .on(STATUS_EVENT, msg => {
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) event - " + STATUS_EVENT);
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) received", msg);
                    if (msg === "connecting") {
                        that.reconnect.delay = fibonacciStrategy.next();
                        that
                            .logger
                            .log("debug", `${LOG_ID} (handleXMPPConnection) update reconnect delay - ${that.reconnect.delay} ms`);
                    } else if (msg === "online" || msg === "disconnect") {
                        fibonacciStrategy.reset();
                        that.reconnect.delay = fibonacciStrategy.getInitialDelay();
                    }
                });

            this
                .xmppClient
                .on(STANZA_EVENT, function (stanza) {
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) event - " + STANZA_EVENT);
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) received", stanza.toString());
                    ...
                    switch (stanza.getName()) {
                        case "iq":
                            ...
                            children.forEach(function (node) {
                                switch (node.getName()) {
                                    ...
                                    case "default":
                                        that
                                            .logger
                                            .log("warn", LOG_ID + "(handleXMPPConnection) not managed - 'stanza'", node.getName());
                                        break;
                                    default:
                                        that
                                            .logger
                            ...
                            if (stanza.attrs.type && stanza.attrs.type === "result") {
                                if (stanza.attrs.id === "enable_xmpp_carbon") {
                                    that
                                        .eventEmitter
                                        .emit("rainbow_oncarbonactivated");
                                }
                            }
                            break;
                        case "message":
                            ...
                            if (stanza.attrs.type === "chat" || stanza.attrs.type === "groupchat") {
                                ...
                                children.forEach(function (node) {
                                    switch (node.getName()) {
                                        case "active":
                                            that
                                                .logger
                                                .log("info", LOG_ID + "(handleXMPPConnection) message - someone is active");
                                            break;
                                        case "inactive":
                                            that
                                                .logger
                                                .log("info", LOG_ID + "(handleXMPPConnection) message - someone is inactive");
                                            break;
                                        case "composing":
                                            that
                                                .logger
                                                .log("info", LOG_ID + "(handleXMPPConnection) message - someone is writing");
                                            break;
                                        case "received":
                                            var receipt = {
                                            ...
                                            that
                                                .logger
                                                .log("info", LOG_ID + "(handleXMPPConnection) message - receipt received");
                                            that
                                                .eventEmitter
                                        ...
                                        case "body":
                                            ...
                                            that
                                                .logger
                                                .log("info", LOG_ID + "(handleXMPPConnection) message - content", "***");
                                        ...
                                        case "request":
                                            ...
                                            that
                                                .logger
                                                .log("info", LOG_ID + "(handleXMPPConnection) answered - send receipt 'received'", stanzaReceived.root().toString());
                                            that
                                                .xmppClient
                                                .send(stanzaReceived);
                                            //Acknowledge 'read'
                                            if (that.shouldSendReadReceipt || (stanza.attrs.type === "groupchat" && XMPPUtils.getResourceFromFullJID(stanza.attrs.from) === that.fullJid)) {
                                                ...
                                                that
                                                    .logger
                                                    .log("info", LOG_ID + "(handleXMPPConnection) answered - send receipt 'read'", stanzaRead.root().toString());
                                                that
                                                    .xmppClient
                                                    .send(stanzaRead);
                                            }
                                        ...
                                        case "x":
                                            ...
                                            if (xmlns === "jabber:x:conference") {
                                                ...
                                                that
                                                    .logger
                                                    .log("info", LOG_ID + "(handleXMPPConnection) invitation received");
                                                that
                                                    .eventEmitter
                                                    .emit("rainbow_invitationreceived", invitation);
                                                break;
                                            }
                                            break;
                                        default:
                                ...
                                if (hasATextMessage && ((stanza.attrs.type === "groupchat" && fromBubbleUserJid !== that.fullJid) || (stanza.attrs.type === "chat" && fromJid !== that.fullJid))) {
                                    that
                                        .logger
                                        .log("info", LOG_ID + "(handleXMPPConnection) message - chat message received");
                        ...
                        default:
                            that
                                .logger
                                .log("warn", LOG_ID + "(handleXMPPConnection) not managed - 'stanza'", stanza.getName());
                            break;
                    }
                });
            ...
            this
                .xmppClient
                .on(ERROR_EVENT, function (err) {
                    if (err.code === "HPE_INVALID_CONSTANT") {
                        return;
                    }
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + ERROR_EVENT);
                    that
                        .logger
                        .log("error", LOG_ID + "(handleXMPPConnection) received", err.condition || err);
                    that
                        .eventEmitter
                        .emit("rainbow_onxmpperror", err);
                });

            this
                .xmppClient
                .on(OFFLINE_EVENT, function (msg) {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + OFFLINE_EVENT);
                    that
                        .logger
                        .log("warn", LOG_ID + "(handleXMPPConnection) received", msg);
                });

            this
                .xmppClient
                .on(CONNECT_EVENT, function (msg) {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + CONNECT_EVENT);
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) received", msg);
                });

            this
                .xmppClient
                .on(RECONNECT_EVENT, function (msg) {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + RECONNECT_EVENT);
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) received", msg);
                });
            ...
            this
                .xmppClient
                .on(DISCONNECT_EVENT, function () {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + DISCONNECT_EVENT);
                });

            this
                .xmppClient
                .on(CLOSE_EVENT, function (msg) {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + CLOSE_EVENT);
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) received", msg);
                });
            ...
            this
                .xmppClient
                .on(END_EVENT, function (msg) {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - " + END_EVENT);
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) received", msg);
                });
            ...
            this
                .reconnect
                .entity
                .on(RECONNECTING_EVENT, () => {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - reconnecting");
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) reconnecting");
                });

            this
                .reconnect
                .entity
                .on(RECONNECTED_EVENT, () => {
                    that
                        .logger
                        .log("debug", LOG_ID + "(handleXMPPConnection) event - reconnected");
                    that
                        .logger
                        .log("info", LOG_ID + "(handleXMPPConnection) reconnected");
                });
        ...
        return new Promise(function (resolve, reject) {
            try {
                if (withXMPP) {
                    that
                        .logger
                        .log("debug", LOG_ID + "(start) host used", that.host);
                    that
                        .logger
                        .log("info", LOG_ID + "(start) XMPP URL", that.serverUR);
                } else {
                    that
                        .logger
                        .log("info", LOG_ID + "(start) XMPP connection blocked by configuration");
                }
    ...
    signin(account) {
        ...
        return new Promise(function (resolve) {
            if (that.useXMPP) {
                that
                    .logger
                    .log("debug", LOG_ID + "(signin) _entering_");
                ...
                that
                    .logger
                    .log("info", LOG_ID + "(signin) account used", that.jid_im);

                handleXMPPConnection();
                that
                    .logger
                    .log("debug", LOG_ID + "(signin) _exiting_");
            } else {
                that
                    .eventEmitter
                    .emit("rainbow_xmppfakeconnected");
            }
            resolve();
        });
    }
