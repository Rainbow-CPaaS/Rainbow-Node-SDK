...
class XMPPService {
    ...
    constructor(_xmpp, _im, _eventEmitter, _logger, _proxy) {
        ...
        handleXMPPConnection = () => {
            ...
            this.xmppClient.on(STANZA_EVENT, (stanza) => {
                ...
                let delivered = PubSub.publish( stanza.getNS() + "." + stanza.getName() + (stanza.attrs.type ? "." +  stanza.attrs.type: ""), stanza);

                stanza.children.forEach( ( child ) => {
                    delivered += PubSub.publish( child.getNS() + "." + stanza.getName() + (stanza.attrs.type ? "." +  stanza.attrs.type: ""), stanza);
                });
    ...
    signin(account) {
        ...
        return new Promise(function(resolve) {
            ...
            if (that.useXMPP) {
                ...
                that.IQEventHandlerToken = [ 
                    PubSub.subscribe(that.IQEventHandler.IQ_GET, that.IQEventHandler.onIqGetReceived),
                    PubSub.subscribe(that.IQEventHandler.IQ_RESULT, that.IQEventHandler.onIqResultReceived)
                ];
        ...
        if(!this.forceClose) {
            this.idleTimer = setTimeout( () => {
                ...
                this.pingTimer = setTimeout( () => {
                    ...
                    this.eventEmitter.emit("rainbow_onxmpperror", err);  
    ...
    mamQuery( jid, options) {
        ...
        that.xmppClient.send(stanza, onComplete);
    }
    ...
    mamQueryMuc(jid, to, options) {
        ...
        let stanza = xml("iq", {
            ...
            type: "submit"
                ...
                "var": "FORM_TYPE",
                "type": "hidden"
            }), _p.filter( (key) => options[key]).map((key) => {
                    let value = xml("field", {
                        "var": key
                    }, xml("value", options[key]));
                    delete options[key];
        ...
        that.xmppClient.sendReceive(stanza, onComplete);
    }
    ...
    mamDelete(jid, options) {
        ...
        let mamAttr = {xmlns: Strophe.NS.MAM};
        if (!!options["deleteid"]) {
