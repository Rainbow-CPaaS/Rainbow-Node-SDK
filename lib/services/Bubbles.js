...
class Bubbles {
    ...
    createBubble(strName, strDescription) {
        ...
        if(!strName) {
            this._logger.log("debug", LOG_ID + "(createBubble) bad 'strName' parameter", strName);
            return Error.BAD_REQUEST;
        }

        if(!strDescription) {
            this._logger.log("debug", LOG_ID + "(createBubble) bad 'strDescription' parameter", strDescription);
            return Error.BAD_REQUEST;
        }
    ...
    deleteBubble(bubble) {
        ...
        if(!bubble) {
            this._logger.log("debug", LOG_ID + "(deleteBubble) bad 'bubble' parameter", bubble);
            return Error.BAD_REQUEST;
        }
    ...
    leaveBubble(bubble) {
        ...
        if(!bubble) {
            this._logger.log("debug", LOG_ID + "(leaveBubble) bad 'bubble' parameter", bubble);
            return Error.BAD_REQUEST;
        }
    ...
    inviteContactToBubble(contact, bubble, boolIsModerator, boolWithInvitation, strReason) {
        ...
        if(!contact) {
            this._logger.log("debug", LOG_ID + "(inviteContactToBubble) bad 'contact' parameter", contact);
            return Error.BAD_REQUEST;
        }

        if(!bubble) {
            this._logger.log("debug", LOG_ID + "(inviteContactToBubble) bad 'bubble' parameter", bubble);
            return Error.BAD_REQUEST;
        }
        ...
        return new Promise(function(resolve, reject) {
            ...
            that._rest.inviteContactToBubble(contact.id, bubble.id, boolIsModerator, boolWithInvitation, strReason).then(function(invitation) {
                ...
                that._rest.getBubble(bubble.id).then(function(bubbleUpdated) {
                    ...
                    var foundIndex = that._bubbles.findIndex(bubble => bubble.id === bubbleUpdated.id);
    ...
    removeContactFromBubble(contact, bubble) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (!contact) {
                this._logger.log("debug", LOG_ID + "(removeContactFromBubble) bad 'contact' parameter", contact);
                reject(Error.BAD_REQUEST);
            } else if (!bubble) {
                this._logger.log("debug", LOG_ID + "(removeContactFromBubble) bad 'bubble' parameter", bubble);
                reject(Error.BAD_REQUEST);
            } else {
                ...
                switch (contactStatus) {
                    ...
                    case "accepted":
                        ...
                        break;
                    default:
                        that._logger.log("warning", LOG_ID + "(removeContactFromBubble) contact not found in that bubble");
                        resolve(bubble);
                        break;
                }
    ...
    getBubbleById(strId) {
        ...
        if(!strId) {
            this._logger.log("debug", LOG_ID + "(getBubbleById) bad 'strId' parameter", strId);
            return Error.BAD_REQUEST;
        }
    ...
    getBubbleByJid(strJid) {
        ...
        if(!strJid) {
            this._logger.log("debug", LOG_ID + "(getBubbleByJid) bad 'strJid' parameter", strJid);
            return Error.BAD_REQUEST;
        }
