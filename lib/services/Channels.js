...
class Channels {
    ...
    createChannel(name, title, visibility, max_items, max_payload_size) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!name) {
                ...
                that._rest.createBubble(name, title, visibility, max_items, max_payload_size).then((channel) => {
     ...
     * @memberof Channels
     */
    findChannel(title_substring, limit, offset, sortField, sortOrder) {
        ...
        if (!title_substring) {
            this._logger.log("debug", LOG_ID + "(findChannel) bad or empty 'title_substring' parameter ", title_substring);
            throw new Error(ErrorCase.BAD_REQUEST.msg);
        }

        return new Promise((resolve, reject) => {

            that._rest.findChannel(title_substring, limit, offset, sortField, sortOrder).then((channel) => {
                that._logger.log("info", LOG_ID + "(findChannel) channel set", channel.name);
                resolve(channel);
            }).catch((err) => {
                that._logger.log("error", LOG_ID + "(findChannel) error", err);
                that._logger.log("debug", LOG_ID + "(findChannel) _exiting_");
                reject(err);
            });
        });
    }
    ...
    updateChannel(channelId, title, visibility, max_items, max_payload_size) {
        ...
        return new Promise((resolve, reject) => {
            ...
            that._rest.updateChannel(channelId, title, visibility, max_items, max_payload_size).then((channel) => {
                that._logger.log("info", LOG_ID + "(updateChannel) channel updated", status);
                resolve(channel);
            }).catch((err) => {
