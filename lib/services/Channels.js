...
class Channels {
    ...
    start(_xmpp, _rest) {
        ...
        return new Promise(function(resolve, reject) {
            try {
                ...
                that._rest = _rest;
                that._channels = [];
                that._eventEmitter.on("rainbow_onchannelmessagereceived", that._onChannelMessageReceived.bind(that));
     ...
     * @param {boolean} [isLimitedToCompany=false] True to limit the visibility to the company.
     * @return {Promise<Channel>} New Channel
     * @memberOf Channel
     * @description
     *  Create a new public channel
     */
    createChannel(name, description, isLimitedToCompany) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!name) {
                ...
                let max_items = 30;
                let max_payload_size = 60000;
                let visibility = isLimitedToCompany ? "company" : "public";

                this._rest.createChannel(name, description, visibility, max_items, max_payload_size).then((channel) => {
     ...
     * @instance
     * @async
     * @param {String} name Search this provided substring in the channel titles (case insensitive).
    ...
    findChannel(name) {
        ...
        if (!name) {
            this._logger.log("debug", LOG_ID + "(findChannel) bad or empty 'name' parameter ", name);
            throw new Error(ErrorCase.BAD_REQUEST.msg);
        }

        return new Promise((resolve, reject) => {

            this._rest.findChannels(name).then((channels) => {
     ...
     * @return {Promise<Users[]>} The channel users
     * @description
    ...
    getUsersFromChannel(channel) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this._rest.getChannelUsers(channel.id).then((users) => {
    ...
    addMembersToChannel(channel, members) {
        ...
        if (!channel) {
            this._logger.log("debug", LOG_ID + "(addMembersToChannel) bad or empty 'channel' parameter", channel);
            throw new Error(ErrorCase.BAD_REQUEST.msg);
        }

        if (!members) {
            this._logger.log("debug", LOG_ID + "(addMembersToChannel) bad or empty 'members' parameter", members);
            throw new Error(ErrorCase.BAD_REQUEST.msg);
        }
