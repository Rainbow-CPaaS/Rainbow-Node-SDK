...
class Conversations {
     ...
     * @param data
     * @param origMsgId
     * @returns {Promise<String>} id the id of the message correction message. Throw an error if the send fails.
     */
    async sendCorrectedChatMessage(conversation, data, origMsgId) {
        let that = this;

        let originalMessage = conversation.getMessageById(origMsgId);
        if (originalMessage.from !== that._rest.loggedInUser.jid_im) {
        ...
        try {
            ...
            newMsg.originalMessageReplaced = originalMessage;
            originalMessage.replacedByMessage = newMsg; // Warning this a circular depend.
