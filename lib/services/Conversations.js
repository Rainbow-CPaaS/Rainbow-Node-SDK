...
class Conversations {
    ...
    attachHandlers() {
        ...
        that.conversationHistoryHandlerToken = [
            ...
            PubSub.subscribe(that.conversationHistoryHandler.FIN_MAM, that.conversationHistoryHandler.onMamMessageReceived)
        ];
    }
    ...
    getBubbleConversation(roomJid, conversationDbId, lastModification, lastMessageText, missedIMCounter, noError, muted, creationDate, lastMessageSender) {
        let that = this;

        that
            ._logger
            .log("info", LOG_ID + "[conversationService] getBubbleConversation " + roomJid);
        ...
        return new Promise((resolve, reject) => {
            ...
            var bubble = that._bubbles.getBubbleByJid(roomJid);
            if (!bubble) {
                that
                    ._logger
                    .log("error", LOG_ID + "[conversationService] getBubbleConversation (" + roomJid + ") failure : no such bubble");
                ...
                if (conversationDbId) {
                    ...
                    that
                        ...
                        .catch(function (error) {
                            var errorMessage = "getBubbleConversation (" + roomJid + ") failure : " + error.message;
                            that
                                ._logger
    ...
    removeConversation(conversation) {
       ...
       /* that.orderConversations();
        ...
        $rootScope.$broadcast("ON_CONVERSATION_REMOVE_EVENT", conversation.id);
    ...
    onRoomAdminMessageEvent(__event, roomJid, userJid, type, msgId) {
        ...
        if (conversation && contact) {
            ...
            if (conversation.bubble && conversation.bubble.isMeetingRoom()) {
                return;
            }
            this
