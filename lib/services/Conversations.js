        ...
        return new Promise((resolve, reject) => {
            ...
            that._fileStorageService.createFileDescriptor(file.name, fileExtension, file.size, viewers)
                ...
                .then(function successCallback(fileDescriptorResult) {
                        ...
                        that.sendExistingFSMessage(conversation, fileDescriptorResult.fileName, fileDescriptorResult);
                        resolve(message);
                    },
                    function errorCallback(error) {
     ...
     */
     sendExistingFSMessage(conversation, message, fileDescriptor) {
        ...
        if (conversation.type === Conversation.Type.ONE_TO_ONE) {
            ...
            that._xmpp.sendChatExistingFSMessage(unicodeData, to, lang, fileDescriptor);
        } else {
            let to = conversation.room.jid;
            that._xmpp.sendChatExistingFSMessageToBubble(unicodeData, to, lang, fileDescriptor);
        }
/*
        var unicodeData = message.data;

        var xmppMessage = null;
        let that = this;
        // Build the message ID
        var messageToSendID = message.id;

        // Handle One to one conversation message
        if (conversation.type === Conversation.Type.ONE_TO_ONE) {
            var to = conversation.contact.jid;
            xmppMessage = $msg({ to: to, type: "chat", id: messageToSendID, "xml:lang": contactService.currentLanguage })
        ...
        // Handle Room conversation message
            ...
            xmppMessage = $msg({ to: conversation.room.jid, type: "groupchat", id: messageToSendID })
                .c("body", { "xml:lang": contactService.currentLanguage }).t(unicodeData).up()
                .c("request", { "xmlns": Conversation.ReceiptNS }).up()
                .c("active", { "xmlns": Conversation.ChatstatesNS }).up();
        }
        ...
        xmppMessage.c("x", {
            ...
            .c("url", {}, url)
            .c("mime", {}, fileDescriptor.typeMIME)
            .c("filename", {}, fileDescriptor.fileName)
            .c("size", {}, fileDescriptor.size).up().up()
            .c("store", {
