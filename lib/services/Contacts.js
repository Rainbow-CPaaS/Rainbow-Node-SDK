...
class Contacts {
     ...
     * @description
     *  Get a contact by his JID
     */
    getContactByJid(strJid) {

        if(!strJid) {
            this.logger.log("warn", LOG_ID + "(getContactByJid) bad or empty 'strJid' parameter", strJid);
    ...
    getContactById(strId) {
        ...
        if(!strId) {
            this.logger.log("warn", LOG_ID + "(getContactById) bad or empty 'strId' parameter", strId);
            return Error.BAD_REQUEST;
        }
