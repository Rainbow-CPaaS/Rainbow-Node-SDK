...
const LOG_ID = "FILESTORAGESERVICE - ";
...
class FileStorageService {
    ...
    createFileDescriptor(name, extension, size, viewers) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.$http({
                ...
                .then((response ) => {
                    const fileDescriptor = this.createFileDescriptorFromData(response.data);
    ...
    createFileDescriptorFromData(data) {
        ...
        if (data) {
            ...
            let fd =  this.fileDescriptorFactory(data.id, url, data.ownerId, data.fileName, data.extension, data.typeMIME,
        ...
        return new Promise((resolve, reject) => {
            this.rest.receivedFileDescriptors("full", 1000)
                ...
                .catch((errorResponse) => {
                    ...
                    reject(errorResponse);
                    hat.logger.log("error", LOG_ID + "(retrieveFileDescriptorsListPerOwner) " + errorResponse);
                });
        });
    }
     ...
     * @returns {ng.IPromise<FileDescriptor[]>} : list of received files descriptors 
     * 
     * @memberOf FileStorageService
     */
    retrieveFilesReceivedFromPeer(userId, peerId) {
        ...
        return new Promise((resolve, reject) => {
            this.rest.retrieveFilesReceivedFromPeer()
                .then((response) => {
     ...
     * @returns {ng.IPromise<FileDescriptor>} file descriptor with newly added viewer
     * 
     * @memberOf FileStorageService
     */
    addFileViewer(fileId, viewerId, viewerType) {
        ...
        return new Promise((resolve, reject) => {
            this.rest.addFileViewer(fileId, viewerId, viewerTyp).then(
                (response) => {
     ...
     * @returns {ng.IPromise<FileDescriptor>} file descriptor retrieved
     * 
     * @memberOf FileStorageService
     */
    retrieveOneFileDescriptor(fileId: string): ng.IPromise < FileDescriptor > {
     ...
     * 
     * @param {string} fileId [required] Identifier of file descriptor to retrieve
     * @returns {ng.IPromise<FileDescriptor>} file descriptor retrieved or null if none found
     * 
     * @memberOf FileStorageService
     */
    retrieveAndStoreOneFileDescriptor(fileId: string, forceRetrieve: boolean): ng.IPromise < any > {
        ...
        return this.retrieveOneFileDescriptor(fileId)
            .then((retrievedFileDescriptor: FileDescriptor) => {
                ...
                let oldFileDescriptorIndex = this.helpersService.findIndex(this.fileDescriptors, (_fileDescriptor) => {
                ...
                let oldReceivedFileDescriptorIndex = this.helpersService.findIndex(this.receivedFileDescriptors, (_fileDescriptor) => {
                    return _fileDescriptor.id === retrievedFileDescriptor.id;
                });
                ...
                this.orderDocuments();
                this.$rootScope.$broadcast("ON_FILE_DESCRIPTOR_RECEIVED_EVENT", {
                    fileId: retrievedFileDescriptor.id
                });
            ...
            .catch((errorResponse) => {
                ...
                if (error.status >= 400 && error.status < 500) {
                    if (fileDescriptor) {
                        ...
                        this.$rootScope.$broadcast("ON_FILE_REMOVED_FROM_STORE_EVENT", {
    ...
    // SharePubliclyAFile(fileId: string): ng.IPromise<{}> {
    //     
    //     return this.$q((resolve, reject) => {
    //         this.$http({
    //             method: "POST",
    ...
    // StopThePublicFileSharing(fileId: string): ng.IPromise<{}> {
    //     return this.$q((resolve, reject) => {
    //         this.$http({
    //             method: "DELETE",
