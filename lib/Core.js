    ...
    constructor(options) {
        ...
        _signin = (forceStopXMPP) => {
            ...
            return new Promise(function(resolve, reject) {
                ...
                that._xmpp.stop(forceStopXMPP).then(() => {
        ...
        _retrieveInformation = (useCLIMode) => {
        ...
        this._eventEmitter.iee.on("rainbow_application_token_updated", function(token) {
            that._rest.applicationToken = token;
        });

        this._eventEmitter.iee.on("rainbow_onxmpperror", function() {
            that._stateManager.transitTo(that._stateManager.ERROR);
        });
            ...
            try {
                ...
                if (!that.options.hasCredentials) {
                    ...
                    that._stateManager.start().then(() => {
                        ...
                        return that._conversations.start(that._xmpp, that._rest, that._contacts);
                    }).then(() => {
    ...
    signin(forceStopXMPP) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            _signin(forceStopXMPP).then(function(_json) {
                ...
                _tokenSurvey();
                that._stateManager.transitTo(that._stateManager.CONNECTED);
                return _retrieveInformation(that.options.useCLIMode);
            }).then(()=> {
                that._stateManager.transitTo(that._stateManager.READY);
                resolve(json);
            }).catch(function(err) {
