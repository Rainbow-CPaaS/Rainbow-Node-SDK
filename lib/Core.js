...
class Core {
    ...
    constructor(options) {
        ...
        this._CPaaSService = new CPaaSService(this._http, this._eventEmitter.iee, this.logger);
        ...
        try
        {
            ...
            return new Promise(function(resolve, reject) {
                if (!that.options.hasCredentials) {
                    ...
                    Promise.all([
                        that._http.start(),
                        that._CPaaSService.start(that.options.application.id, that.options.application.secret, that.options.httpOptions.host)
                        .then( (data) => {
                            return that._rest.start(that._http, data.token);
                        }),
