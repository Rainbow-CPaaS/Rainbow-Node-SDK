...
var getHTTPOptions, getXMPPOptions, getProxyOptions;
    ...
    constructor(options, _eventEmitter) {
        ...
        getProxyOptions = (options) => {
            ...
            if(!("host" in options.proxy)) {
                this.logger.log("warn", LOG_ID + "(constructor) 'host' property is not valid. No proxy will be used", options.proxy.host);
            }
            else {
                proxyOptions.host = options.proxy.host;
            }
            if(!("port" in options.proxy)) {
                this.logger.log("info", LOG_ID + "(constructor) 'port' property is not valid. Use default 80", options.proxy.port);
            }
            else {
                proxyOptions.port = options.proxy.port;
            }
            if(!("protocol" in options.proxy)) {
                this.logger.log("info", LOG_ID + "(constructor) 'protocol' property not specified. Use default 'http'", options.proxy.protocol);
            }
            else {
        ...
        if(!options.rainbow || !options.rainbow.host) {
            this.logger.log("warn", LOG_ID + "(constructor) No 'host' property in 'options'. Use default: 'sandbox'. Check the documentation to configure it");
            options.rainbow = {host: 'sandbox'};
        }

        if(!options.proxy) {
            this.logger.log("info", LOG_ID + "(constructor) No 'proxy' property in 'options. Use default: no proxy. Check the documentation to enable it");
            options.proxy = {host: "", protocol: "http", port: 80};
        }

        if(!options.credentials) {
            this.logger.log("error", LOG_ID + "(constructor) No 'credentials' property in 'options'. Can't sign-in. Check the documentation to configure it");
            this.eventEmitter.emit("rainbow_nocredentials");
        } else if(!options.credentials.login || !options.credentials.password) {
            this.logger.log("error", LOG_ID + "(constructor) No 'login' or 'password' property in 'options'. Can't sign-in. Check the documentation to configure it");
            this.eventEmitter.emit("rainbow_nocredentials");
        } else {
        ...
        this._xmpp = new XMPPService(getXMPPOptions(options), this.eventEmitter, this.logger, this._proxy);
