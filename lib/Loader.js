...
class Loader {
    ...
    constructor(options, _eventEmitter) {
        ...
        if(!options || !options.rainbow || !options.rainbow.host) {
            winston.warn("error", LOG_ID + "constructor - host is missing from the configuration file. Use 'sandbox'. Check the documentation!");
        }
        ...
        if(!options.credentials || !options.credentials.login || !options.credentials.password) {
            winston.error("error", LOG_ID + "constructor - 'login' or 'password' parameters are missing from the configuration file. Check the documentation!");
        }
